...
import { NextResponse } from "next/server";
import { prisma } from "@/lib/db";
// XÓA DÒNG IMPORT TRANSFORMERS Ở ĐÂY ĐỂ HẾT LỖI 500

// =========================================================
// 1. NẠP VECTOR TĨNH (GIỮ NGUYÊN)
// =========================================================
let PRECOMPUTED_VECTORS: Record<string, number[]> = {};
try {
    PRECOMPUTED_VECTORS = require('@/data/style-vectors.json');
} catch (e) {
    console.log("⚠️ API: Không có file vector tĩnh.");
}
...
const STYLE_RULES: Record<string, any> = {
    'nang-tho': { mustInclude: ['nữ', 'váy', 'đầm'], mustExclude: ['nam'] },
    'nam-tinh': { mustInclude: ['nam'], mustExclude: ['nữ'] },
    // ... các rules khác bạn giữ nguyên như cũ
};
...
export async function GET(request: Request) {
  try {
    ...
    if (style || keyword) {
        let queryVector: number[] = [];
        let filterSql = `embedding IS NOT NULL`;

        // 1. Áp dụng filter (Giữ nguyên logic của bạn)
        if (style && STYLE_RULES[style]) {
            const rule = STYLE_RULES[style];
            if (rule.mustInclude) filterSql += ` AND (${rule.mustInclude.map((w:any) => `name ILIKE '%${w}%'`).join(' OR ')})`;
            if (rule.mustExclude) filterSql += ` AND (${rule.mustExclude.map((w:any) => `name NOT ILIKE '%${w}%'`).join(' AND ')})`;
        }

        // 2. LẤY VECTOR (ĐÃ ĐỔI SANG GỌI LOCAL SERVER)
        if (style && PRECOMPUTED_VECTORS[style]) {
            queryVector = PRECOMPUTED_VECTORS[style];
        } else {
            // GỌI VỀ MÁY NHÀ NHỜ TÍNH VECTOR
            ...
            try {
                ...
                queryVector = aiData.vector;
            } catch (err) {
                console.error("❌ Không gọi được AI máy nhà:", err);
                return NextResponse.json({ data: [] }); // Fallback rỗng nếu máy nhà tắt
            }
        }

        // 3. Thực hiện Query (Giữ nguyên logic của bạn)
        ...
        // Logic lấy danh sách thường (Giữ nguyên)
        products = await prisma.product.findMany({
            take: limit,
            skip: offset,
            orderBy: type === "bestseller" ? { sold_count_value: 'desc' } : { updated_at: 'desc' }
        });
    }

    // --- MAPPING DATA (Giữ nguyên logic cực tốt của bạn) ---
    const safeData = products.map((p: any) => {
        const strId = p.itemid?.toString() || "0";
        let imageUrl = p.image || "";
        if (imageUrl && !imageUrl.startsWith('http')) {
             imageUrl = `https://down-vn.img.susercontent.com/file/${imageUrl}`;
        }
        return {
            ...
            sold: p.sold_count_value || 0,
            rating: p.rating_value || 5.0,
            image: imageUrl,
            affiliate_link: p.affiliate_link,
            label: (p.seller_flag as any)?.name === 'OFFICIAL_SHOP' ? 'Mall' : ''
        };
    });
